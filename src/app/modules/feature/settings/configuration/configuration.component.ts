...
export class ConfigurationComponent implements OnInit {
  ...
  async getPersonalFormData() {
    try {
      const response = await this.configurationService.getPersonalFormData(this.userId).toPromise();
      this.createPersonalForm(response?.content?.[0] || {});
  ...
  validateUsername(control: AbstractControl): Observable<any> {
    if (control.valueChanges && control.dirty) {
      return control.valueChanges.pipe(
        ...
        switchMap(value => this.configurationService.validateUserName(value, this.userId).pipe(
  ...
  async updateDisplayPicture(files: FileList, type: string) {
    try {
      if (files?.length) {
        ...
        const response = await this.configurationService.updateDisplayPicture(type, formData).toPromise() as any;
        if (response?.image) {
  ...
  async onSubmit(formType) {
    if (this[formType].valid) {
      ...
      try {
        ...
        const response = await this.configurationService.updateProfileDetails({
          ...this[formType].value,
  ...
  async uploadCertificates(files: FileList, inputRef: HTMLInputElement) {
    try {
      if (files?.length) {
        ...
        const response = await this.configurationService.uploadCertificates(formData).toPromise();
